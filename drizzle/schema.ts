...
export const financialPeriods = mysqlTable("financial_periods", {
	...
	closedBy: int().references(() => users.id),
	reopenedAt: timestamp({ mode: "string" }),
	reopenedBy: int().references(() => users.id),
...
export const financialControlReviews = mysqlTable("financial_control_reviews", {
	id: int().autoincrement().primaryKey().notNull(),
	periodId: int().notNull().references(() => financialPeriods.id, { onDelete: "cascade" }),
...
export const financialPeriodActions = mysqlTable("financial_period_actions", {
	...
	actionBy: int().notNull().references(() => users.id),
},
	(table) => [
...
export const invoices = mysqlTable("invoices", {
	...
	taxAmount: decimal({ precision: 15, scale: 2 }).notNull(),
	totalAmount: decimal({ precision: 15, scale: 2 }).notNull(),
	paidAmount: decimal({ precision: 15, scale: 2 }).default('0.00').notNull(),
	balanceDue: decimal({ precision: 15, scale: 2 }).notNull(),
...
export const users = mysqlTable("users", {
	...
	role: mysqlEnum(['admin','farm_manager','accountant','sales_staff','production_worker','chicken_house_operator']).default('production_worker').notNull(),
	createdAt: timestamp({ mode: 'string' }).default('CURRENT_TIMESTAMP').notNull(),
	updatedAt: timestamp({ mode: 'string' }).defaultNow().onUpdateNow().notNull(),
