...
try {
  ...
  console.log('Deleting from flock_vaccination_schedules...');
  await connection.query('DELETE FROM flock_vaccination_schedules WHERE flockId = ?', [flockId]);
  
  console.log('Deleting from flock_stress_pack_schedules...');
  await connection.query('DELETE FROM flock_stress_pack_schedules WHERE flockId = ?', [flockId]);
