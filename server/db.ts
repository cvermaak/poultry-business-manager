...
export async function createInvoice(data: {
  ...
  const exclusiveTotal = parseFloat((data.totalWeight * data.pricePerKgExcl).toFixed(2));
  const vatAmount = parseFloat((exclusiveTotal * (data.vatPercentage / 100)).toFixed(2));
  const inclusiveTotal = parseFloat((exclusiveTotal + vatAmount).toFixed(2));
  ...
  const result = await db.insert(invoices).values({
    ...
    subtotal: exclusiveTotal,  // DECIMAL - not multiplied by 100
    taxAmount: vatAmount,      // DECIMAL - not multiplied by 100
    totalAmount: inclusiveTotal, // DECIMAL - not multiplied by 100
    paidAmount: 0,
    balanceDue: inclusiveTotal,
    status: "draft",
    createdBy: data.createdBy,
    ...
    // NOTE: createdAt and updatedAt are auto-generated by database, don't set them
  });
  ...
  try {
    ...
    const lineItemResult = await db.insert(invoiceItems).values({
      ...
      pricePerUnit: data.pricePerKgExcl,
      discount: 0,
      vatPercentage: data.vatPercentage,
      amount: inclusiveTotal,
      // NOTE: createdAt and updatedAt are auto-generated by database, don't set them
    });
    console.log(`[DEBUG] Line item created successfully`);
  } catch (error) {
    console.error('[DEBUG] Error creating line item:', error);
    // Don't throw - invoice was created even if line item fails
  }
...
export async function updateCompanySettings(data: any, userId: number) {
  ...
  if (existing) {
    return await db
      ...
      .set({
        ...data,
        // NOTE: updatedAt is auto-updated by database with ON UPDATE CURRENT_TIMESTAMP
      })
